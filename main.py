...
#     with torch.no_grad():
#         outputs = nermodel(tokens['input_ids'].unsqueeze(0).to(device),tokens['attention_mask'].unsqueeze(0).to(device))  # Forward pass through the model
...
def predict_entities(request: TextRequest):
    ...
    predictions = torch.argmax(outputs, dim=-1).squeeze().tolist()  # Get the predicted labels
    all_preds = [ner_idx2label[idx] for idx in predictions]

    all_preds = all_preds[1:sum(tokens['attention_mask'].tolist())]  # Remove the [CLS] and [SEP] tokens
...
def predict_entities(request: TextRequest):
    ...
    with torch.no_grad():
        outputs = posmodel(tokens['input_ids'].unsqueeze(0).to(device),tokens['attention_mask'].unsqueeze(0).to(device))  # Forward pass through the model
